...
namespace UnityEngine.AIGraph
{
    ...
    {
        public const string IconsPath = "Packages/cn.tuanjie.ai.graph/Editor/Resources/Icons/";
        ...
        public static Texture2D RefreshIcon
        {
            get
            {
                ...
                {
                    _refreshIcon = Resources.Load<Texture2D>("Icons/refresh");
                }
        ...
        public static Texture2D SaveIcon
        {
            get
            {
                if (_saveIcon == null)
                {
                    _saveIcon = Resources.Load<Texture2D>("Icons/Icon-CodeSave");
                }
    ...
    public class SDUtil
    {
        ...
        private const string LOG = "TJAI Graph: ";
    ...
    public class SDControlNetInfo
    {
        public string modes;
        public string preprocess_modes;
    ...
    {
        public string prompt="";
        public string[] ch_prompt;
        public string[] en_prompt;
    }
    [Serializable]
    public class TJAIStyle
    {
        public string style_guid;
        public float style_weight = 0.85f;
    }
    [Serializable]
    public class CtrlUnit
    {
        public int ctrl_mode;
        public int ctrl_preprocess_mode;
        public string ctrl_img;
        public float ctrl_weight;
    }

    [Serializable]
    public class ControlNetTypes
    {
        public Dictionary<string, ControlNetStruct> control_types;
    }

    [Serializable]
    public class ControlNetStruct
    {
...
#if UNITY_EDITOR
    /// 在对应节点的视图类中（xxxNodeView.cs），Enable() 方法内调用 RegisterNodeChangingCallback(string fieldName) 
    /// 可自动跟踪对应名称的字段，如果该字段的数值有变化，底层会自动触发 NodeChanging 事件；字段支持基础类型和一些 Unity
    /// 内置类型（https://docs.unity3d.com/cn/2022.3/Manual/UIE-binding-data-type-conversion.html），但不直接支持
    /// 自定义类，解决方法是为自定义类实现自定义的 PropertyDrawer 类
    /// 
    /// 以后对于节点内一些自定义类型的字段（比如 SDTxt2ImgNode.cs 内的 public HiresUpscaler upscaler）
    /// 可以考虑利用 Unity 原生提供的 PropertyDrawer 自定义对应的控制面板，这样在具体用到该类型的节点内，就不用再
    /// 重复去写界面相关的逻辑了；而且 TJAIGraph 的底层（BaseNodeView.cs）会自动为这样的字段注册回调函数，确保在这
    /// 些字段的值被修改时触发 NodeChanging 事件，就无需再遍历所有回调函数，手动添加 NotifyNodeChanging() 了
    /// 
    /// 下面是对 HiresUpscaler 类型自定义 PropertyDrawer 的样例，经测试是有效的，其他类型可以此做参考
    //[CustomPropertyDrawer(typeof(HiresUpscaler))]
    //public class HiresUpscalerDrawer : PropertyDrawer
    //{
    ...
    //        var hr_upscaler = new PropertyField(property.FindPropertyRelative("hr_upscaler"));
    //        var hr_second_pass_steps = new PropertyField(property.FindPropertyRelative("hr_second_pass_steps"));
    //        var denoising_strength = new PropertyField(property.FindPropertyRelative("denoising_strength"));
    //        var hr_scale = new PropertyField(property.FindPropertyRelative("hr_scale"));
    //        var hr_resize_x = new PropertyField(property.FindPropertyRelative("hr_resize_x"));
    //        var hr_resize_y = new PropertyField(property.FindPropertyRelative("hr_resize_y"));

    //        // Add fields to the container.
    //        container.Add(hr_upscaler);
    //        container.Add(hr_second_pass_steps);
    //        container.Add(denoising_strength);
    //        container.Add(hr_scale);
    //        container.Add(hr_resize_x);
    //        container.Add(hr_resize_y);
