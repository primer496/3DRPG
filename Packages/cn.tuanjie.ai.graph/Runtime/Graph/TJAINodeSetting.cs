...
namespace UnityEngine.AIGraph
{
    ...
    public class TJAINodeSetting
    {
        ...
        [NonSerialized] internal TJAINodeSetting resolvedSettings;

        public static TJAINodeSetting defaultValue
        {
            get => new TJAINodeSetting()
            {
                widthScale = 1.0f,
                heightScale = 1.0f,
                depthScale = 1.0f,
        ...
        public void ResolveAndUpdate(SDNode node)
        {
            ...
            if (resolvedSettings == this || resolvedSettings == null)
                resolvedSettings = new TJAINodeSetting();
            ...
            int ResolveWidth(SDNode node, TJAIGraph graph)
            {
            ...
            int ResolveHeight(SDNode node, TJAIGraph graph)
            {
                switch (node.settings.sizeMode)
                {
                    default:
            ...
            int ResolveDepth(SDNode node, TJAIGraph graph)
            {
            ...
            OutputPrecision ResolveOutputPrecision(SDNode node, TJAIGraph graph)
            {
            ...
            OutputChannel ResolveOutputChannels(SDNode node, TJAIGraph graph)
            {
            ...
            OutputDimension ResolveTextureDimension(SDNode node, TJAIGraph graph)
            {
            ...
            OutputWrapMode ResolveWrapMode(SDNode node, TJAIGraph graph)
            {
            ...
            OutputFilterMode ResolveFilterMode(SDNode node, TJAIGraph graph)
            {
                // if this function is called from the output node and the dimension is default, then we set it to a default value
        ...
        public bool IsHDR(TJAIGraph graph)
        {
        ...
        public GraphicsFormat GetGraphicsFormat(TJAIGraph graph)
            => ConvertToGraphicsFormat(GetResolvedPrecision(graph));

        public int GetResolvedWidth(TJAIGraph graph) => resolvedSettings.width;
        public int GetResolvedHeight(TJAIGraph graph) => resolvedSettings.height;
        public int GetResolvedDepth(TJAIGraph graph) => resolvedSettings.depth;
        public OutputPrecision GetResolvedPrecision(TJAIGraph graph) => resolvedSettings.outputPrecision;
        public OutputChannel GetResolvedChannels(TJAIGraph graph) => resolvedSettings.outputChannels;

        public TextureDimension GetResolvedTextureDimension(TJAIGraph graph) =>
            (TextureDimension)resolvedSettings.dimension;

        public TextureWrapMode GetResolvedWrapMode(TJAIGraph graph) =>
            (TextureWrapMode)resolvedSettings.wrapMode;

        public FilterMode GetResolvedFilterMode(TJAIGraph graph) => (FilterMode)resolvedSettings.filterMode;

        public bool NeedsUpdate(TJAIGraph graph, Texture t, bool checkFormat = true)
        {
        ...
        public TJAINodeSetting Clone()
        {
            return new TJAINodeSetting
            {
