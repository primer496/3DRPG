...
namespace UnityEngine.AIGraph
{
    ...
    internal class Painter : IDisposable
    {
        ...
        /// <summary>
        /// The painter is seamless or not (repeat mode).
        /// </summary>
        public bool isSeamless { get; set; } = false;
        ...
        class PaintMaterial : IDisposable
        {
            ...
            static readonly int k_Seamless = Shader.PropertyToID("_Seamless");
            ...
            public void SetSeamless(bool seamless) => material.SetFloat(k_Seamless, seamless ? 1f : 0f);
        ...
        public void InitializeWithData(byte[] imageData)
        {
            ...
            if (isClear)
                m_RenderTexture.initializationColor = clearColor;
            else
        ...
        public void Paint(Vector2 startPosition, Vector2 endPosition, Color color)
        {
            ...
            m_PaintMaterial.SetSeamless(isSeamless);
        ...
        void SetUpdateZones(Vector2 startPosition, Vector2 endPosition)
        {
            ...
            var minX = isSeamless ? 0 :            Mathf.Max(Mathf.RoundToInt(Mathf.Min(startPosition.x, endPosition.x) - enlargedBrushRadius), 0);
            var maxX = isSeamless ? m_Size.x - 1 : Mathf.Min(Mathf.RoundToInt(Mathf.Max(startPosition.x, endPosition.x) + enlargedBrushRadius), m_Size.x - 1);
            var minY = isSeamless ? 0 :            Mathf.Max(Mathf.RoundToInt(Mathf.Min(startPosition.y, endPosition.y) - enlargedBrushRadius), 0);
            var maxY = isSeamless ? m_Size.y - 1 : Mathf.Min(Mathf.RoundToInt(Mathf.Max(startPosition.y, endPosition.y) + enlargedBrushRadius), m_Size.y - 1);
