...
namespace GraphProcessor
{
    ...
    public abstract class BaseNode
    {
        ...
        /// <summary>True if the node needs manual triggers to process, false otherwise</summary>
        public virtual bool needTrigger => false;
        ...
        /// <summary>
        /// Is the node in a collapsed group, false means not in a group or in expanded group
        /// </summary>
        public bool inCollapsedGroup = false;
        ...
        /// <summary>
        /// 在节点正式执行前的就绪状态，用于在trigger节点触发时将整条流程的节点都置为pause状态
        /// 事件执行顺序：onReady -> beforeProcessSetup -> onProcessed/onCancelled/onError -> afterProcessCleanup
        /// </summary>
        public event Action onReady;
        /// <summary>
        /// Triggered when the node is processes, 执行成功后调用的事件
        /// </summary>
        public event ProcessDelegate onProcessed;
        /// <summary>
        /// 取消执行状态时调用的事件
        /// </summary>
        public event ProcessDelegate onCancelled;
        /// <summary>
        /// 执行失败时调用的事件
        /// </summary>
        ...
        public event Action<bool> onFocuseUpdated;

        /// <summary>
        /// trigger节点执行前的准备事件，验证输入等
        /// </summary>
        ...
        public event Action<string> onProcessEdge;

        public event Action onResetTriggerButton;
        /// <summary>
        ...
        public virtual void CollectSubAssets() { }

        /// <summary>
        /// for AI Generate button
        /// </summary>
        ...
        public bool UpdatePortsForFieldLocal(string fieldName, bool sendPortUpdatedEvent = true)
        {
            ...
            void AddPortData(PortData portData)
            {
                ...
                if (port == null)
                {
                    int inFieldsCount = nodeFields.Where(pair => pair.Value.input).Count();
                    int index;
                    if (fieldInfo.input)
                        index = inFieldsCount - nodeFields.Keys.ToList().IndexOf(fieldName) - 1;
                    else
                        index = nodeFields.Count - nodeFields.Keys.ToList().IndexOf(fieldName) - 1;
                    InsertPort(index, fieldInfo.input, fieldName, portData);
                    changed = true;
                }
                else
                {
        ...
        void InitializeInOutDatas()
        {
            ...
            foreach (var field in fields)
            {
                ...
                {
                    inputAttribute.name = LocalizationManager.Instance.GetLocalizedText(inputAttribute.name);
                    name = inputAttribute.name;
                }

                if (!String.IsNullOrEmpty(outputAttribute?.name))
                {
                    outputAttribute.name = LocalizationManager.Instance.GetLocalizedText(outputAttribute.name);
        ...
        public virtual void OnEdgeConnected(SerializableEdge edge)
        {
            ...
            if (input)
                ...
                portCollection.Add(edge, edge.inputNode.onProcessEdge);
        ...
        public void OnEdgeDisconnected(SerializableEdge edge)
        {
            ...
            if (input)
                RecursiveRestoreTriggerButton(edge.inputNode);

            onAfterEdgeDisconnected?.Invoke(edge);
        }

        public static void RecursiveRestoreTriggerButton(BaseNode currentNode)
        {
            currentNode.onResetTriggerButton?.Invoke();
            var edgeList = currentNode.GetEdgeList();
            foreach (var item in edgeList.Where(item => item.outputNode == currentNode))
            {
                RecursiveRestoreTriggerButton(item.inputNode);
            }
        }
        ...
        public void InvokeBeforeProcessSetup() => beforeProcessSetup?.Invoke();
        public void InvokeAfterProcessCleanup() => afterProcessCleanup?.Invoke();
        public void InvokeOnReady() => onReady?.Invoke();
        public void InvokeOnProcessed() => onProcessed?.Invoke();
        public void InvokeOnCancelled() => onCancelled?.Invoke();
        public void InvokeOnError(string message) => onError?.Invoke(message);
        public void InvokeOnFocused() => onFocuseUpdated?.Invoke(true);
        public void InvokeOnUnfocused() => onFocuseUpdated?.Invoke(false);
        ...
        {
            // init event, make sure this is called in procedure
            onReady += HandleReady;
            beforeProcessSetup += CheckInput;
            onProcessed += HandleSuccess;
            onError += HandleError;
        ...
        public virtual IEnumerator ProcessAsync()
        {
            throw new NotImplementedException();
        }

        protected virtual IEnumerator ProcessAsync(CommandBuffer cmd)
        {
            throw new NotImplementedException();
        }
        ...
        {
            // update status to init if not done
            if (status != NodeStatus.Done)
            {
                UpdateStatus(NodeStatus.Init);
            }
        }
        ...
        protected void CheckInput()
        {
            foreach (var port in inputPorts)
            {
                foreach (var edge in port.GetEdges())
                {
                    var node = edge.outputNode;
                    if (node.status != NodeStatus.Done)
                        throw new ArgumentException($"{node.GetCustomName()} is not ready yet.", port.fieldName);
        ...
        {
            UpdateStatus(NodeStatus.Error, msg);
            RecursiveRestoreTriggerButton(this);
        }

        protected void HandleSuccess()
        {
            UpdateStatus(NodeStatus.Done);
        }
        ...
        public void InsertPort(int index, bool input, string fieldName, PortData portData)
        {
            // Fixup port data info if needed:
            if (portData.displayType == null)
                portData.displayType = nodeFields[fieldName].info.FieldType;
            ...
            if (input)
            {
                if (index >= inputPorts.Count)
                    ...
                    inputPorts.Insert(index, new NodePort(this, fieldName, portData));
            }
            else
            {
                if (index >= outputPorts.Count)
                    ...
                    outputPorts.Insert(index, new NodePort(this, fieldName, portData));
            }
        }
        ...
        public List<SerializableEdge> GetInputEdges()
        {
            List<SerializableEdge> edges = new List<SerializableEdge>();
            foreach (var port in inputPorts)
                edges.AddRange(port.GetEdges());
            return edges;
        }

        public List<SerializableEdge> GetOutputEdges()
        {
            List<SerializableEdge> edges = new List<SerializableEdge>();
            foreach (var port in outputPorts)
                edges.AddRange(port.GetEdges());
        ...
        /// </summary>
        /// <returns></returns>
        public List<SerializableEdge> GetEdgeList()
        {
        ...
        public void UpdateStatus(NodeStatus newStatus, string message = "", float progress = 100.0f)
        {
            ClearMessages();
            if (newStatus == NodeStatus.Error && !string.IsNullOrWhiteSpace(message))
            {
        ...
        public void SetUniqueCustomName(string customName) => nodeCustomName = graph.nodeNames.ChangeName(GetCustomName(), customName);
        /// <summary>
