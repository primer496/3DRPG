...
namespace GraphProcessor
{
	...
	public class InputAttribute : Attribute
	{
		...
		public InputAttribute(string name = null, bool allowMultiple = false, bool immediateUpdate = false)
		{
			this.name = name;
	...
	public class NodeMenuItemAttribute : Attribute
	{
		...
		{
			// 做NodeMenuItem的本地化
            string[] parts = menuTitle.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < parts.Length; i++)
            {
                parts[i] = LocalizationManager.Instance.GetLocalizedText(parts[i]);
            }
            this.menuTitle = String.Join("/", parts); 
			// Debug.Log(this.menuTitle);
	...
	public class CustomPortInputAttribute : Attribute
	{
		...
		public CustomPortInputAttribute(string fieldName, Type[] inputTypes, bool allowCast = true)
		{
	...
	public class CustomPortOutputAttribute : Attribute
	{
		...
		/// <param name="inputType">type of input of the port</param>
		/// <param name="allowCast">if cast is allowed when connecting an edge</param>
		public CustomPortOutputAttribute(string fieldName, Type[] outputTypes, bool allowCast = true)
		{
			this.fieldName = fieldName;
        ...
        {
            this.fieldName = fieldName;
            outputTypes = new Type[] { outputType };
            this.allowCast = allowCast;
        }
    }
