...
namespace GraphProcessor
{
    public class GraphChanges
    {
        ...
        public BaseNode nodeChanging;
        public BaseNode nodeChanged;
        public BaseNode nodeRenamed;
    ...
    public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        [SerializeReference] internal NodeName nodeNames = new NodeName();
        /// <summary>
        ...
        [NonSerialized]
        private Dictionary<BaseNode, List<SerializableEdge>> immediateUpdateListener;
        ...
        /// <summary>
        /// Triggered when parameter.value is changed, for inspetor etc.
        /// </summary>
        public event Action<ExposedParameter> onExposedParameterValueChanged;
        /// <summary>
        /// Triggered when ExposedParameter display view is changed (input type, param name)
        /// </summary>
        public event Action<ExposedParameter> onExposedParameterDisplayChanged;
        ...
        /// <summary>
        /// Api Trigger Node From Graph
        /// </summary>
        public Action<BaseNode> onScriptTriggerNode;
        /// <summary>
        ...
        public virtual void OnUpdate()
        {
            ...
            foreach (var node in nodes)
            {
                ...
                {
                    node.Update();
                    if (immediateUpdateListener.ContainsKey(node))
                    {
                        foreach (var edge in immediateUpdateListener[node])
                        {
                            edge.outputPort.PushData(edge);
                        }
                    }
                    node.isUpdate = false;
                }
        ...
        void InitializeGraphElements()
        {
            ...
            // FIX: ArgumentNullException: Value cannot be null. nodesPerGUID is not serialized
            nodesPerGUID ??= new();
            ...
            immediateUpdateListener = new Dictionary<BaseNode, List<SerializableEdge>>();
            foreach (var edge in edges.ToList())
            {
                ...
                registInstanceUpdatePort(edge);
            }
        }

        void registInstanceUpdatePort(SerializableEdge edge)
        {
            var field = edge.inputPort.fieldInfo;
            var inputAttr = field.GetCustomAttribute<InputAttribute>();
            if (inputAttr != null && inputAttr.immediateUpdate)
            {
                (immediateUpdateListener.TryGetValue(edge.outputPort.owner, out var list) ? list : (immediateUpdateListener[edge.outputPort.owner] = new List<SerializableEdge>())).Add(edge);
                edge.outputPort.PushData(edge);
            } 
        }
        ...
        public BaseNode AddNode(BaseNode node)
        {
            ...
            nodes.Add(node);
            node.Initialize(this);
            node.SetCustomName(nodeNames.AddNewName(node.name));
        ...
        public void RemoveNode(BaseNode node)
        {
            ...
            nodes.Remove(node);
            nodeNames.RemoveName(node.GetCustomName());
        ...
        /// <summary>
        /// Invoke the onGraphChanges event, can be used as trigger to execute the graph when the input property of a node is changed 
        /// </summary>
        /// <param name="node"></param>
        public void NotifyNodeChanging(BaseNode node) => onGraphChanges?.Invoke(new GraphChanges { nodeChanging = node });

        /// <summary>
        /// Invoke the onGraphChanges event, can be used as trigger to execute the graph when the output of a node is changed 
        /// </summary>
        /// <param name="node"></param>
        public void NotifyNodeChanged(BaseNode node) => onGraphChanges?.Invoke(new GraphChanges { nodeChanged = node });

        /// <summary>
        /// Invoke the onGraphChanges event, can be used as trigger to execute the graph when node is renamed
        /// </summary>
        /// <param name="node"></param>
        public void NotifyNodeRenamed(BaseNode node) => onGraphChanges?.Invoke(new GraphChanges { nodeRenamed = node });
...
#pragma warning restore CS0618
        ...
        public void UpdateExposedParameter(string guid, object value)
        {
            ...
            onExposedParameterValueChanged?.Invoke(param);
        }
        ...
        public void UpdateExposedParameterName(ExposedParameter parameter, string name)
        {
            ...
            onExposedParameterDisplayChanged?.Invoke(parameter);
        }
        ...
        /// <summary>
        /// Update parameter display related business
        /// cond 1: update inspector visibility, <param name="isHidden">if hide in inspector</param>
        /// cond 2: update parameter accessor type, , <param name="accesor">get/set</param>
        /// </summary>
        /// <param name="parameter"></param>
        public void NotifyExposedParameterDisplayChanged(ExposedParameter parameter)
        {
            onExposedParameterDisplayChanged?.Invoke(parameter);
        }
