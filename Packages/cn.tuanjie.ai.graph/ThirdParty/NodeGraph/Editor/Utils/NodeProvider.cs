...
namespace GraphProcessor
{
	public static class NodeProvider
	{
		...
		static Dictionary<string, string> nodeNamePath = new();
		...
		static bool IsNodeCreatedFromAsset(Type nodeType)
		{
		...
		static void AddNodeNamePath(Type type)
		{
			var nodeScriptPath = FindPathFromClassName(type.Name);

			// Try find the class name with Node name at the end
			if (string.IsNullOrEmpty(nodeScriptPath))
				nodeScriptPath = FindPathFromClassName(type.Name + "Node");
			if (!string.IsNullOrEmpty(nodeScriptPath))
				nodeNamePath[type.Name] = nodeScriptPath;
		}
        ...
        public static IEnumerable<(string path, Type type)>	GetNodeMenuEntries(BaseGraph graph = null, bool exceptAssetNodes = true)
		{
			foreach (var node in genericNodes.nodePerMenuTitle)
			{
                // ！！！对于 TextureNode, GameObjectNode 等依靠拖入资产添加的节点来说，暂不能用菜单的方式添加，exceptAssetNodes 默认为 True
				// 但是 BaseGraphView 中需要调用此方法，来注册这些拖入资产类节点的初始化函数，仅在此种情况下 exceptAssetNodes 为 False
                if (exceptAssetNodes && IsNodeCreatedFromAsset(node.Value))
					continue;
		...
		public static string GetNodePath(string nodeClassName)
		{
			nodeNamePath.TryGetValue(nodeClassName, out var path);
			return path;
		}
