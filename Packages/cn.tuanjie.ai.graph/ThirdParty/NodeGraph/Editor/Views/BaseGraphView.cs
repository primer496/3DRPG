...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
		...
		public Dictionary<Group, GroupView>         groupViewsPerGroup = new Dictionary<Group, GroupView>();
		...
		/// <summary>
		/// Triggered when the trigger button of a node is pressed
		/// </summary>
		public event Action< BaseNode > nodeTriggered;

		/// <summary>
		/// Triggered when the pause button of a node is pressed
		/// </summary>
		public event Action nodeCancelled;
		...
		public void InvokeNodeTriggeredCallback(BaseNode node) => nodeTriggered?.Invoke(node);

		public void InvokeNodeCancelledCallback(BaseNode node) => nodeCancelled?.Invoke();

		public void ScriptInvokeNodeTriggeredCallback(BaseNode node) => nodeViewsPerNode[node]?.TriggerButtonClicked();
				...
				changes.elementsToRemove.RemoveAll(e => {
					...
					switch (e)
					{
						...
						case BaseNodeView nodeView:
							...
							bool isPara = typeof(ParameterNodeView).IsAssignableFrom(e.GetType());
							foreach (var pv in nodeView.inputPortViews.Concat(nodeView.outputPortViews))
								if (pv.orientation == Orientation.Vertical || isPara)
						...
						case ExposedParameterFieldView blackboardField:
							...
							blackboardField.parameter.DeleteAsset();
							// delete related parameterNode
							var toDelNodes = graph.nodes.Where(n =>
							{
								if (n is ParameterNode pn)
									return pn.parameterGUID == blackboardField.parameter.guid;
								return false;
							}).ToList();
                            toDelNodes.ForEach(n =>
							{
								BaseNodeView nv = nodeViewsPerNode[n];
								foreach (var pv in nv.inputPortViews.Concat(nv.outputPortViews))
									foreach (var edge in pv.GetEdges().ToList())
										Disconnect(edge);
								nodeInspector.NodeViewRemoved(nv);
                                ExceptionToLog.Call(() => nv.OnRemoved());
								graph.RemoveNode(n);
                            } );
                            UpdateSerializedProperties();
							toDelNodes.ForEach(n => RemoveElement(nodeViewsPerNode[n]));
                            if (Selection.activeObject == nodeInspector)
                                UpdateNodeInspectorSelection();
		...
		void GraphChangesCallback(GraphChanges changes)
		{
			...
			// else if (changes.addedGroups != null)
			// {
			// 	// check if related view is created
			// 	if (!groupViewsPerGroup.ContainsKey(changes.addedGroups))
			// 	{
			// 		AddGroupView(changes.addedGroups);
			// 	}
			// } else if (changes.removedGroups != null)
			// {
			// 	if (groupViewsPerGroup.TryGetValue(changes.removedGroups, out var value))
			// 	{
			// 		RemoveElement(value);
			// 	}
			// } else if (changes.addedNode != null)
			// {
			// 	if (nodeViewsPerNode.ContainsKey(changes.addedNode)) return;
			// 	UpdateSerializedProperties();
			// 	var view = AddNodeView(changes.addedNode);
			// 	view.OnCreated();
			// 	UpdateComputeOrder();
		...
		public override List< Port > GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
		{
			...
			compatiblePorts.AddRange(ports.ToList().Where(p => {
				...
				// if port is invisible
				if (!portView.visible || portView.ClassListContains("collapsed"))
					return false;
		...
		public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
		{
            ...
            BuildStackNodeContextualMenu(evt, 3);
        ...
        protected virtual void BuildStackNodeContextualMenu(ContextualMenuPopulateEvent evt, int menuPosition = -1)
        {
            if (menuPosition == -1)
                menuPosition = evt.menu.MenuItems().Count;
            Vector2 position = (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
            evt.menu.InsertAction(menuPosition, "Create Stack Node", (e) => AddStackNode(new BaseStackNode(position)), DropdownMenuAction.AlwaysEnabled);
        }
		...
		{
			//Debug.Log((e.actionKey ? "Ctrl + " : "") + e.keyCode);
			if (e.actionKey && e.keyCode == KeyCode.S)
			{
				//Debug.Log("saving graph to disk...");
				SaveGraphToDisk();
				e.StopPropagation();
			}
			else if (e.actionKey && (e.keyCode == KeyCode.Delete || e.keyCode == KeyCode.Backspace))
			{
				if (canDeleteSelection)
					DeleteSelectionCallback(AskUser.DontAskUser);
				e.StopPropagation();
			}
			else if (e.actionKey && e.keyCode == KeyCode.D)
			{
				// ctrl + D, duplicate node
				Debug.Log($"duplicate selection: {selection}");
				if (canDuplicateSelection)
					DuplicateSelectionCallback();
				e.StopPropagation();
			}
			else if (nodeViews.Count > 0 && e.actionKey && e.altKey)
			{
		...
		/// <summary>
        /// 拖拽随后在图内释放，此操作会处理两件事情：
        /// 1. 若拖拽的节点来自Parameters，创建对应的节点
        /// 2. 若拖拽的资产有对应的initializeNodeFromObject方法，创建对应的节点
        /// </summary>
		...
		public void OnPause()
		{
			// restore all trigger button
			foreach (var nodeView in nodeViews)
			{
				nodeView.EnableTriggerableView();
			}
		}
		...
		public void Initialize(BaseGraph graph)
		{
			...
			this.graph = graph;
            graph.onScriptTriggerNode -= ScriptInvokeNodeTriggeredCallback;
            graph.onScriptTriggerNode += ScriptInvokeNodeTriggeredCallback;
			...
			foreach (var nodeInfo in NodeProvider.GetNodeMenuEntries(graph, exceptAssetNodes: false))
			{
		...
		{
			var c = block is ProviderGroup ? new ProviderGroupView()  : new GroupView();
            ...
            groupViews.Add(c);
            groupViewsPerGroup[block] = c;
            return c;
		}

		public void RemoveGroup(Group block)
		{
			if (groupViewsPerGroup.TryGetValue(block, out var view))
				RemoveGroupView(view);
			graph.RemoveGroup(block);
		}
		...
		public void RemoveGroupView(GroupView view)
		{
			...
			groupViewsPerGroup.Remove(view.group);
		}
		...
		/// <summary>
		/// remove StackNodeView and remove stackNode in graph at the same time
		/// </summary>
		/// <param name="stackNodeView"></param>
		public void RemoveStackNodeView(BaseStackNodeView stackNodeView)
		{
			stackNodeViews.Remove(stackNodeView);
			RemoveElement(stackNodeView);
			// bug fix: to make sure data in graph is matched
			graph.RemoveStackNode(stackNodeView.stackNode);
		}
        ...
        /// <summary>
		/// remove stickNoteView and stickyNote in group at the same time
		/// </summary>
		...
		public void RemoveStickyNoteView(StickyNoteView view)
		{
			...
			RemoveElement(view);
			// bug fix: make sure data in graph is matched
			graph.RemoveStickyNote(view.note);
		}
