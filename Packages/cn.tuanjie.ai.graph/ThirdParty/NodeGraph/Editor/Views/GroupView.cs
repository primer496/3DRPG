...
namespace GraphProcessor
{
    public class GroupView : UnityEditor.Experimental.GraphView.Group
	{
        ...
        protected bool noPicking = false;
        ...
        public GroupView()
        {
            ...
            VisualElement dropArea = contentContainer?.Q("dropArea");
            if (noPicking)
                dropArea.pickingMode = PickingMode.Ignore;
        }
        ...
        /// <summary>
        /// collapse all nodeview in group
        /// </summary>
        void RefreshCollapseState()
        {
            // step 1: check GroupView class
            RemoveFromClassList(group.expanded ? "collapsed" : "expanded");
            AddToClassList(group.expanded ? "expanded" : "collapsed");
            // step 2: refresh NodeView (only display PortView with outer connection)
            ...
            foreach (var nodeGUID in group.innerNodeGUIDs.ToList())
            {
                ...
                var node = owner.graph.nodesPerGUID[nodeGUID];
                var nodeView = owner.nodeViewsPerNode[node];
                nodeView.RefreshCollapseStateInGroup(group, ref posOffset);
                nodeView.RefreshPorts();
            }
            // step 3: forbid drag&drop in collapse state
            VisualElement contentContainer = this.Q("contentContainerPlaceholder");
            VisualElement dropArea = contentContainer?.Q("dropArea");
            if (!noPicking && dropArea != null)
                dropArea.pickingMode = group.expanded ? PickingMode.Position : PickingMode.Ignore;
        }
        ...
        public void UpdateGroupColor(Color newColor)
        {
            ...
            collapseButton.style.backgroundColor = newColor;
        }
