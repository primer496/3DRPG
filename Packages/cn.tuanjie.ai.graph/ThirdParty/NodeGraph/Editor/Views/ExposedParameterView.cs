...
namespace GraphProcessor
{
	public class ExposedParameterView : PinnedElementView
	{
        ...
        protected override void Initialize(BaseGraphView graphView)
        {
            ...
            header.Add(new UnityEngine.UIElements.Button(OnAddClicked){
                text = "+"
            });
        }
        ...
        public void HighlightParameterEvent(string parameterGUID)
        {
            int selectIndex = this.Query<ExposedParameterFieldView>().ToList().FindIndex(e => e.parameter.guid == parameterGUID);
            content[selectIndex].Q<VisualElement>("node-border").AddToClassList("highlighted");
            IVisualElementScheduledItem highlightScheduleItem;
            highlightScheduleItem = this.schedule.Execute(e => {
                content[selectIndex].Q<VisualElement>("node-border").RemoveFromClassList("highlighted");
            }).StartingIn(1000);
        }
    }
}