...
namespace GraphProcessor
{
    ...
    public class ExposedParameterFieldFactory : IDisposable
    {
        ...
        /// <summary>
        /// only update title/accesor, remain value
        /// </summary>
        /// <param name="parameters"></param>
        public void UpdateParamsDisplay(ExposedParameter param = null)
        {
            if (param != null)
            {
                // for display update
                var workParam = exposedParameterObject.parameters.FirstOrDefault(n => n.guid == param.guid);
                if (workParam != null)
                {
                    workParam.name = param.name;
                    workParam.settings = param.settings;
                }
            }
            else
            {
                // for parameter list update
                HashSet<string> oldGuids = exposedParameterObject.parameters.Select(n => n.guid).ToHashSet();
                HashSet<string> curGuids = graph.exposedParameters.Select(n => n.guid).ToHashSet();
                foreach (string guid in oldGuids.Except(curGuids))
                {
                    // remove
                    var toDelParams = exposedParameterObject.parameters.Where(n => n.guid == guid);
                    foreach (ExposedParameter toDelParam in toDelParams)
                    {
                        toDelParam.DeleteAsset();
                    }
                    exposedParameterObject.parameters.RemoveAll(n => n.guid == guid);
                }
                foreach (string guid in curGuids.Except(oldGuids))
                {
                    // add new node
                    var curParam = graph.exposedParameters.FirstOrDefault(n => n.guid == guid);
                    exposedParameterObject.parameters.Add(curParam.Clone());
                }
            }
            serializedObject.Update();
            serializedParameters = serializedObject.FindProperty(nameof(ExposedParameterWorkaround.parameters));
        ...
        {
            exposedParameterObject.parameters.ForEach(p => p.DeleteAsset());
        }
    }
}