...
## 附：对话系统遇到的一个常见 UI 错误与解决经验

问题现象：第一次打开对话 UI 时一切正常（文本、选项和分支都能走）；第二次再打开对话 UI 时，面板显示回到默认文本或默认选项，无法更新为当前节点内容。

根因分析（简短）：Unity 的 `UIDocument` 在其宿主 GameObject 被禁用（`SetActive(false)`）再启用时会重建整个 VisualTree，之前通过 `rootVisualElement.Q(...)` 保存的 `VisualElement` 引用会变成“已分离/过期”的实例。第一次打开时引用有效；当对话结束时若调用了 `view.gameObject.SetActive(false)`，UIDocument 的 VisualTree 被销毁；再次激活时 Unity 会创建新的 VisualElement 实例，但控制器仍在使用旧引用写数据，导致 UI 不更新。

解决办法（要点）：

- 把每次需要最新 VisualElement 的查询与回调注册从 `Awake()` 移到 `OnEnable()`，这样每次 GameObject 被启用时都会重新 `root = uiDocument.rootVisualElement;` 并 `BindComponents()`/`RegisterEvents()`，确保拿到新的元素并绑定它们的事件。
- 把与 Presenter 绑定的长期事件（例如 `BindPresenter` 中把 Presenter 的回调注册到 `UnityEvent`）保留在 `Awake()`，避免重复注册；但在 `OnDisable()` 中应解除 UI 回调注册或移除之前注册的 VisualElement 回调，防止重复触发或内存泄露。

推荐代码实践（简短清单）：

1. `Awake()`：仅做一次性的引用分配（例如 `uiDocument = GetComponent<UIDocument>()`）和 Presenter 的绑定（`BindPresenter`）。
2. `OnEnable()`：`root = uiDocument.rootVisualElement;` → `BindComponents();` → `RegisterEvents();`（重新查询并注册）
3. `OnDisable()`：解绑 VisualElement 的回调或清理引用（如果使用 `RegisterCallback`，在此处使用 `UnregisterCallback` 或手动清理委托）。
4. 如果需要在 `Enable/Disable` 间保持同一组 `UnityEvent` 监听器（例如 Presenter 绑定），确保不会在 `OnEnable`/`BindComponents` 重复调用 `OnOptionClicked.AddListener(...)`。

修复效果：每次对话 UI 被启用时都会获得新的 VisualElement 引用并重新绑定事件，避免因为使用过期引用而导致 UI 不更新的问题。

最终正确做法（已采用）：UI Toolkit 的正确显隐方式是 `root.style.display = DisplayStyle.None/Flex`，即调用已封装好的 `view.ShowDialogue(false/true)`，**永远不要对挂有 UIDocument 的 GameObject 调用 SetActive(false)**。这样 VisualTree 始终存在，`Awake()` 中绑定的引用和事件一直有效，无需在 `OnEnable()` 中重复绑定。与之对比：UGUI 的 `SetActive(false)` 只停止渲染，不会销毁组件引用，所以 UGUI 中使用 SetActive 控制显隐是完全正常的。
