...
# include <sys/stat.h>
# include <errno.h>
# if UNITY_DEVELOPER_BUILD && (defined(HOST_ANDROID) || defined(LINUX))
#   define PR_SET_VMA_SUPPORTED 1
#   include <sys/prctl.h>
#   ifndef PR_SET_VMA
...
# define SBRK_ARG_T ptrdiff_t
    ...
    void* UNITY_MMAP(void* addr, size_t length, int prot, int flags, int fd, off_t offset)
    {
        void* ptr = mmap(addr, length, prot, flags, fd, offset);
#if PR_SET_VMA_SUPPORTED
        if (ptr != MAP_FAILED && fd == -1)
            prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, ptr, length, "UNITY_MEMORY_MMAP_BDWGC");
#endif
        return ptr;
    }
...
#   endif
    ...
    result = UNITY_MMAP(last_addr, bytes, (PROT_READ | PROT_WRITE)
...
#         endif
#       else
          void * result = UNITY_MMAP(start_addr, len, PROT_NONE,
...
#       elif defined(NACL)
          ...
          void *result = UNITY_MMAP(start_addr, len, (PROT_READ | PROT_WRITE)
                                    | (GC_pages_executable ? PROT_EXEC : 0),
                                   MAP_PRIVATE | MAP_FIXED | OPT_MAP_ANON,
                                   zero_fd, 0 /* offset */);
          if (result != (void *)start_addr)
